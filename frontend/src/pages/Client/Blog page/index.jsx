...
import { useGetAllBlogsQuery, useGetByIdBlogQuery } from "../../../redux/rtk query/Slices/blogSlice";
import { Helmet } from "react-helmet";
import LoaderIcon from "../../../components/Loaders/Loader";
import moment from 'moment';
import { useGetAllUserQuery, useGetByIdUserQuery, useGetFavoriteBlogsUserQuery } from "../../../redux/rtk query/Slices/userSlice";
...
const BlogPage = () => {
    ...
    let { data: Blog1Id } = useGetSettingsByKeyQuery('Blog1Id')
    let { data: Blog2Id } = useGetSettingsByKeyQuery('Blog2Id')
    let { data: Blog3Id } = useGetSettingsByKeyQuery('Blog3Id')
    let { data: Blog1Data } = useGetByIdBlogQuery(Blog1Id?.value)
    let { data: Blog2Data } = useGetByIdBlogQuery(Blog2Id?.value)
    let { data: Blog3Data } = useGetByIdBlogQuery(Blog3Id?.value)
                    ...
                    <div className="bg-[var(--bg-color)] py-[130px] text-[var(--text-color)]">
                        <Container>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-[2fr_3fr] lg:grid-cols-[1fr_2fr] gap-3 sm:gap-[50px] relative">
                                <div className="flex">
                                    <div className="order-first w-full sm:h-[80vh] sm:sticky sm:top-[17%] sm:right-[7.5%] p-4 cursor-pointer rounded-4 bg-[var(--movies-bg)] shadow-lg overflow-y-scroll">
                                        <div className="flex flex-col gap-3">
                                            {
                                                Blog1Data || Blog2Data || Blog3Data ? (
                                                    [Blog1Data, Blog2Data, Blog3Data].map((blog) => {
                                                        if (blog?.isApproved) {
                                                            let user = allUsers?.find(user => user.id === blog?.authorId);
                                                            let userPhoto = allAvatar?.find(avatar => avatar.id === user?.avatarId)
                                                            let time = moment.utc(blog?.createdAt.split('.')[0]).local().format('DD/MMM/YYYY HH:mm');
                                                            return (
                                                                <div key={blog.id} onClick={() => { navigate(`${blog.id}`) }} className="cursor-pointer p-3 rounded-4 bg-[var(--movies-bg)] transition-all duration-250 ease-in hover:shadow-gray-500 hover:shadow-lg border-2 border-[var(--bg-color)]">
                                                                    ...
                                                                    <div className="flex gap-2 justify-end">
                                                                        ...
                                                                        <div onClick={(e) => navigate(`/user/${user.userName}`, e.stopPropagation())} className='relative flex items-center justify-center w-[65px] h-[65px]'>
