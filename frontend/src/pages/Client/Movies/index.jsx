...
export default function MoviesPage() {
    ...
    useEffect(() => {
        if (!isLoading && allData) {
            setAllMoviesData(allData.filter(item => item.isFilm === true))
        }
    }, [allData, isLoading])
    ...
    const handleSort = (value) => {
        if (value == 'default') {
            ...
            let sortedAZ = allMoviesData.toSorted((a, b) => a.name.localeCompare(b.name))
            setAllMoviesData(sortedAZ)
        } else if (value == 'z-a') {
            let sortedZA = allMoviesData.toSorted((a, b) => b.name.localeCompare(a.name))
            setAllMoviesData(sortedZA)
        } else if (value == 'fromMoreToLessImdb') {
            let sortedFromMoreToLessImdb = allMoviesData.toSorted((a, b) => a.imdb - b.imdb)
            setAllMoviesData(sortedFromMoreToLessImdb)
        } else if (value == 'fromLessToMoreImdb') {
            let sortedFromLessToMoreImdb = allMoviesData.toSorted((a, b) => b.imdb - a.imdb)
            setAllMoviesData(sortedFromLessToMoreImdb)
        }
    }
    ...
    const getMoviesByGenre = (genreId) => {
        ...
        let myGenreData = allData.filter(item => item.isFilm === true)
                        ...
                        <Container>
                            ...
                            focus:outline-none focus:shadow-[0_0px_5px_0px_#fff]' />
                                        ...
                                        <div className='relative'>
                                            ...
                                            <ul
                                                className='absolute top-[100%] rounded-4 bg-white text-black opacity-90 w-full z-10 transition-all duration-200 ease-in handleBars'
                                                ref={sortRef}>
                                    ...
                                    allGenre.length ? (
                                        <div className='mt-3'>
                                            ...
                                            <div ref={genreRef} className='flex flex-wrap genreListHidden'>
                                                <div onClick={() => setAllMoviesData(allData.filter(item => item.isFilm === true))}
                                ...
                                <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 items-center justify-center'>
                                    {
                                        allData.filter(item => item.isFilm === true) ? (
                                            ...
                                            <h1 className='font-["Kanit"]'>Film yoxdur</h1>
                                        )
                                    }
                                </div>
                            </div>
